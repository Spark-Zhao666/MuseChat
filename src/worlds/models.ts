...
> = {
  ...
  rose: { materials: ["Red", "Green"] },
  // Plant: {materials: ["Green"] },
  // Rabbit: { materials: ["Brown", "White", "Black"] },
};
...
export function getModelPathsAndMaterials(
  ...
  if (model.versions) {
    for (let i = 1; i <= model.versions; i++) {
      filePaths.push(
        `${basePath}${collections[collection].name}/${name}_${i}.glb`,
      );
    }
  } else {
    filePaths.push(`${basePath}${collections[collection].name}/${name}.glb`);
  }
